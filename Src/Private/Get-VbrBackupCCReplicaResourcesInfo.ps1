            ...
            if ($CloudObjects = Get-VBRCloudHardwarePlan | Sort-Object -Property Name) {
                foreach ($CloudObject in $CloudObjects) {
                    ...
                    $TempBackupCCReplicaResourcesInfo = [PSCustomObject]@{
                        ...
                        Label = Get-DiaNodeIcon -Name "$((Remove-SpecialChar -String $CloudObject.Name.split(".")[0] -SpecialChars '\').toUpper())" -IconType "VBR_Hardware_Resources" -Align "Center" -Rows $AditionalInfo -ImagesObj $Images -IconDebug $IconDebug -FontSize 18
                        Id = $CloudObject.Id
