            ...
            if ($TapeDrives) {
                foreach ($TapeDrive in $TapeDrives) {
                    ...
                    $TempBackupTapeDriveInfo = [PSCustomObject]@{
                        Name = $TapeDrive.Name
                        Label = Get-DiaNodeIcon -Name "$((Remove-SpecialChar -String ("Drive $($TapeDrive.Address + 1)").split(".")[0] -SpecialChars '\').toUpper())" -IconType 'VBR_Tape_Drive' -Align "Center" -Rows $Rows -ImagesObj $Images -IconDebug $IconDebug -fontSize 18
