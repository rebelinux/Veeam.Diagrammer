        ...
        try {
            ...
            if ($BackupServerInfo) {
                if ($VMwareBackupProxy) {
                    ...
                    Node ViProxies @{Label = (Add-DiaHtmlNodeTable -ImagesObj $Images -inputObject ($VMwareBackupProxy | ForEach-Object { $_.Name.split('.')[0] }) -Align "Center" -iconType "VBR_Proxy_Server" -ColumnSize $VMwareBackupProxyColumnSize -IconDebug $IconDebug -MultiIcon -AditionalInfo $VMwareBackupProxy.AditionalInfo -Subgraph -SubgraphIconType "VBR_Proxy" -SubgraphLabel "VMware Backup Proxies" -SubgraphLabelFontSize 26 -SubgraphLabelPos "top" -SubgraphTableStyle "dashed,rounded" -FontColor $Fontcolor -TableBorderColor $Edgecolor -TableBorder "1" -FontSize 18 -SubgraphFontBold); shape = 'plain'; fontsize = 14; fontname = "Segoe Ui" }
                ...
                if ($vSphereObj) {
                    ...
                    foreach ($vCenter in $vSphereObj) {
                        ...
                        try {
                            if ($ViClustersChildsNodes) {
                                ...
                                $ViClustersNodes += Add-DiaHtmlSubGraph -ImagesObj $Images -TableArray $ViClustersChildsNodes -Align 'Center' -IconDebug $IconDebug -Label 'Clusters' -LabelPos "top" -FontColor $Fontcolor -TableStyle "dashed,rounded" -TableBorderColor $Edgecolor -TableBorder "1" -ColumnSize $ViClustersChildsNodesColumnSize -FontSize 20 -FontBold
                    ...
                    try {
                        if ($VivCenterNodes) {
                            ...
                            $VivCenterNodesAll += Add-DiaHtmlSubGraph -ImagesObj $Images -TableArray $VivCenterNodes -Align 'Center' -IconDebug $IconDebug -Label 'Management Servers' -LabelPos "top" -FontColor $Fontcolor -TableStyle "dashed,rounded" -TableBorderColor $Edgecolor -TableBorder "1" -ColumnSize $VivCenterNodesColumnSize -FontSize 24 -FontBold
                        }
                    } catch {
                        Write-Verbose "Error: Unable to create vCenter Server Objects. Disabling the section"
                        Write-Debug "Error Message: $($_.Exception.Message)"
                ...
                if ($vSphereServerObj = Get-VbrBackupvSphereStandAloneInfo | Sort-Object) {
                    ...
                    if ($ViStandAloneNodes) {
                        try {
                            $VivCenterNodesAll += Add-DiaHtmlSubGraph -ImagesObj $Images -TableArray $ViStandAloneNodes -Align 'Center' -IconDebug $IconDebug -Label 'ESxi StandAlone Hosts' -LabelPos "top" -FontColor $Fontcolor -TableStyle "dashed,rounded" -TableBorderColor $Edgecolor -TableBorder "1" -ColumnSize 1 -FontSize 24 -FontBold
                        } catch {
