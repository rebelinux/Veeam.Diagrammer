            ...
            if ($CloudObjects = (Get-VBRCloudTenant).Resources | Sort-Object -Property Name) {
                foreach ($CloudObject in ($CloudObjects.Repository | Sort-Object -Property Name -Unique)) {
                    ...
                    $TempBackupCCBKStorageInfo = [PSCustomObject]@{
                        ...
                        Label = Add-DiaNodeIcon -Name "$((Remove-SpecialChar -String $CloudObject.Name.split(".")[0] -SpecialChars '\').toUpper())" -IconType $Type -Align "Center" -AditionalInfo $AditionalInfo -ImagesObj $Images -IconDebug $IconDebug -FontSize 18 -FontBold
                        Id = $CloudObject.Id
