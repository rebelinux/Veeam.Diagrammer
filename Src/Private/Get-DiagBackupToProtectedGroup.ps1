                ...
                if ($BackupServerInfo) {
                    if ($FileBackupProxy) {
                        ...
                        SubGraph MainSubGraphFileProxy -Attributes $ProxiesAttr -ScriptBlock {
                            ...
                            Node FileProxies @{Label = (Get-DiaHTMLNodeTable -ImagesObj $Images -inputObject ($FileBackupProxy | ForEach-Object { $_.Name.split('.')[0] }) -Align "Center" -iconType "VBR_Proxy_Server" -columnSize $columnSize -IconDebug $IconDebug -MultiIcon -AditionalInfo ($FileBackupProxy.AditionalInfo )); shape = 'plain'; fontsize = 14; fontname = "Segoe Ui" }
