            ...
            if ($TapeLibraries) {
                foreach ($TapeLibrary in $TapeLibraries) {
                    ...
                    $TempBackupTapelibraryInfo = [PSCustomObject]@{
                        ...
                        Label = Get-DiaNodeIcon -Name "$((Remove-SpecialChar -String $TapeLibrary.Name.split(".")[0] -SpecialChars '\').toUpper())" -IconType 'VBR_Tape_Library' -Align "Center" -Rows $Rows -ImagesObj $Images -IconDebug $IconDebug -fontSize 18
