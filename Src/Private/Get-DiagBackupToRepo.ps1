                ...
                if ($BackupRepo) {
                    SubGraph LocalRepos -Attributes @{Label='Local Repository'; fontsize=18; penwidth=1.5; labelloc='b'} {
                        foreach ($REPOOBJ in $LocalBackupRepo) {
                            ...
                            node $REPOOBJ -NodeScript {$_.Name} (Get-ImageIcon @REPOHASHTABLE)
                        }
                        rank $LocalBackupRepo.Name -NodeScript {$_.Name}
                    }
                    SubGraph RemoteRepos -Attributes @{Label='Deduplicating Storage Appliances'; fontsize=18; penwidth=1.5; labelloc='b'} {
